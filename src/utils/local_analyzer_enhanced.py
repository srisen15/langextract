...
class LocalTestAnalyzer:
        ...
        try:
            ...
            if use_enhanced:
                ...
                if generate_reports:
                    ...
                    report_files = self.generate_enhanced_reports(
                        enhanced_results, performance_analysis, improvements, business_analysis, failure_analysis)
            ...
            for feature, data in sorted(feature_breakdown.items(), key=lambda x: x[1]['total'], reverse=True):
                if data['total'] > 0:
                    print(f"   - {feature.replace('_',
                                                  ' ').title()}: {data['total']} tests ({data['pass_rate']:.1f}% pass rate)")
            print()
    ...
    def generate_enhanced_reports(self, enhanced_results: List[Dict[str, Any]],
        ...
        try:
            ...
            report_content = self.generate_enhanced_report_content(
                enhanced_results, performance_analysis, improvements, business_analysis, failure_analysis)
            ...
            business_report = self.generate_business_impact_report(
                business_analysis, failure_analysis, enhanced_results)
    ...
    def generate_enhanced_report_content(self, enhanced_results: List[Dict[str, Any]],
        ...
        if performance_analysis and 'error' not in performance_analysis:
            ...
            report.append(
                f"- **Average Duration**: {performance_analysis.get('average_duration_seconds', 0):.1f} seconds")
            report.append(
                f"- **Total Execution Time**: {performance_analysis.get('total_execution_time_minutes', 0):.1f} minutes")
        ...
        if business_analysis:
            ...
            if 'feature_breakdown' in business_analysis:
                ...
                for feature, data in business_analysis['feature_breakdown'].items():
                    ...
                    report.append(
                        f"- {health_status} **{feature}**: {data['pass_rate']:.1f}% ({data['passed']}/{data['total']} tests)")
                report.append("")
    ...
    def generate_business_impact_report(self, business_analysis: Dict[str, Any],
        ...
        report.append(
            f"- **Quality Gate Status**: {'✅ PASSED' if failed_tests < total_tests * 0.1 else '⚠️ ATTENTION NEEDED'}")
        report.append(
            f"- **Business Risk Level**: {self._assess_business_risk(business_analysis, failed_tests, total_tests)}")
        report.append("")
        ...
        gates = [("Overall Pass Rate",
                  overall_pass_rate,
                  95,
                  "% passed"),
                 ("Critical Flow Health",
                  min([h['health_score'] for h in critical_flows.values()]) if critical_flows else 100,
                  90,
                  "% healthy"),
                 ("Feature Coverage",
                  len([f for f in feature_breakdown.values() if f['total'] > 0]),
                  len(feature_breakdown),
                  " features covered")]
        ...
        report.append("---")
        report.append(
            f"*Report generated by Enhanced Test Analyzer v2.1.0 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
