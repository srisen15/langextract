...
class PerformanceAnalyzer:
    ...
    def _generate_performance_recommendations(self, durations: List[int], distribution: Dict[str, int]) -> List[str]:
        ...
        if very_slow_count > len(durations) * 0.1:  # More than 10% very slow
            recommendations.append(
                f"{very_slow_count} tests take over 1 minute. Review these for optimization opportunities.")
    ...
    def generate_performance_report(self, analysis: Dict[str, Any]) -> str:
        ...
        if insights:
            ...
            report.append(
                f"**Fastest Test**: `{fastest.get('name', 'N/A')}` - {fastest.get('duration_seconds', 0):.2f}s")

            slowest = insights.get('slowest_test', {})
            report.append(
                f"**Slowest Test**: `{slowest.get('name', 'N/A')}` - {slowest.get('duration_seconds', 0):.2f}s")
            report.append("")
