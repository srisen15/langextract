...
class TestLogExtractor:
    ...
    def categorize_failure(self) -> tuple[FailureCategory, Priority]:
        ...
        if any(
            keyword in failure_message for keyword in [
                'unauthorized',
                'forbidden',
                'authentication',
                'login',
        ...
        if any(
            keyword in failure_message for keyword in [
                'timeout',
                'timed out',
                'waiting for']) or 'timeout' in status_trace:
    ...
    def extract_basic_info(self) -> TestExecutionSummary:
        ...
        test_file = self.test_data.get(
            'fullName', '').split(':')[0] if ':' in self.test_data.get(
            'fullName', '') else ''
...
"""
        ...
        key_interactions = [ui for ui in ui_interactions if any(keyword in ui['action'] for keyword in [
                                                                'Navigate', 'Click getByRole', 'Fill', 'Expect "toContain"'])][:10]
