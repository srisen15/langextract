...
class EnhancedTestLogExtractor(TestLogExtractor):
    ...
    def enhanced_categorize_failure(self) -> tuple[FailureCategory, Priority, str]:
        ...
        if any(pattern in failure_message for pattern in network_patterns) or any(
                error in failure_message for error in http_errors):
        ...
        if is_flaky:
            return FailureCategory.FLAKY_TEST, Priority.LOW, f"Flaky test detected ({
                self.test_data.get(
                    'retriesCount', 0)} retries)"

        # 10. Enhanced unknown categorization with analysis
        return self._analyze_unknown_failure(failure_message, status_trace)

    def _analyze_unknown_failure(self, failure_message: str,
                                 status_trace: str) -> tuple[FailureCategory, Priority, str]:
        ...
        priority = Priority.HIGH if any(
            critical in failure_message for critical in [
                'fatal', 'critical', 'severe']) else Priority.MEDIUM
