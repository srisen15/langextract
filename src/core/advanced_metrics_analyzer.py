...
class AdvancedMetricsAnalyzer:
        ...
        for flow_name, patterns in self.critical_flows.items():
            ...
            if flow_tests:
                ...
                critical_flow_health[flow_name] = {
                    ...
                    'risk_level': self._calculate_risk_level(passed_flow_tests / total_flow_tests * 100 if total_flow_tests > 0 else 0)
                }
    ...
    def _generate_performance_recommendations(self, benchmark_analysis: Dict, slow_tests: List) -> List[str]:
        ...
        for test_type, analysis in benchmark_analysis.items():
            if analysis['performance_status'] == 'Needs Improvement':
                recommendations.append(f"{test_type.replace('_', ' ').title()} are running {analysis['vs_target']} - optimize or review thresholds")
        
        if len(slow_tests) > 5:
            recommendations.append(f"{len(slow_tests)} tests taking >1 minute - consider breaking into smaller tests or parallelization")
    ...
    def _generate_failure_insights(self, failure_patterns: Dict, top_keywords: List) -> List[str]:
        ...
        if timing_issues > 0:
            insights.append(f"{timing_issues} tests failing on timing - consider performance optimization or wait strategy review")
