...
class QualityGates:
    ...
    def check_gates(self, results: dict) -> tuple[bool, list]:
        ...
        if results['failure_rate'] > self.gates['max_failure_rate']:
            failures.append(
                f"Failure rate ({
                    results['failure_rate']:.1f}%) exceeds threshold ({
                    self.gates['max_failure_rate']}%)")

        # Critical failures check
        if results['critical_failures'] > self.gates['max_critical_failures']:
            failures.append(
                f"Critical failures ({
                    results['critical_failures']}) exceed threshold ({
                    self.gates['max_critical_failures']})")
        ...
        if results['high_priority_failures'] > self.gates['max_high_priority_failures']:
            failures.append(
                f"High priority failures ({
                    results['high_priority_failures']}) exceed threshold ({
                    self.gates['max_high_priority_failures']})")
        ...
        if flaky_rate > self.gates['max_flaky_test_rate']:
            failures.append(
                f"Flaky test rate ({
                    flaky_rate:.1f}%) exceeds threshold ({
                    self.gates['max_flaky_test_rate']}%)")
        ...
        if categories.get('Timeout', 0) > self.gates['max_timeout_failures']:
            failures.append(
                f"Timeout failures ({
                    categories['Timeout']}) exceed threshold ({
                    self.gates['max_timeout_failures']})")

        if categories.get('Data/State Issue', 0) > self.gates['max_data_issue_failures']:
            failures.append(
                f"Data issue failures ({categories['Data/State Issue']}) exceed threshold ({self.gates['max_data_issue_failures']})")

        if categories.get('Authentication Error', 0) > self.gates['max_auth_failures']:
            failures.append(
                f"Authentication failures ({
                    categories['Authentication Error']}) exceed threshold ({
                    self.gates['max_auth_failures']})")
...
class CIPlatformIntegration:
    ...
    def _jenkins_annotations(results: dict):
        ...
        if results['critical_failures'] > 0 or results['high_priority_failures'] > 0:
            print(
                f"JENKINS_BUILD_DESCRIPTION=Critical: {
                    results['critical_failures']}, High Priority: {
                    results['high_priority_failures']} failures")
