...
def analyze_csv_results(csv_file):
    ...
    results['failure_rate'] = (results['failed_tests'] / results['total_tests'] * 100) if results['total_tests'] > 0 else 0
...
def determine_build_status(results, max_failure_rate, max_critical_failures, max_high_priority):
    ...
    if results['high_priority_failures'] > max_high_priority:
        reasons.append(f"High priority failures ({results['high_priority_failures']}) exceed threshold ({max_high_priority})")
